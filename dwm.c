  ...
  drw_text(drw, m->ww - tw, 0, tw, bh, 0, stext, 0);
	...
	for (i = 0; i < LENGTH(tags); i++) {
      ...
      if (m == selmon && selmon->sel && selmon->sel->tags & 1 << i) {
        drw_rect(drw, x + ulinepadsel, bh - ulinestroke - ulinevoffset, w - (ulinepadsel * 2), ulinestroke, 1, 0);
      } else {
...
focusmaster(const Arg *arg)
{
	...
	if (selmon->sel == master) {
      ...
      restack(selmon);
      XWarpPointer(dpy, None, selmon->tagmarked[i]->win, 0, 0, 0, 0, selmon->tagmarked[i]->w/2, selmon->tagmarked[i]->h/2);
    }
	} else {
...
monocle(Monitor *m)
{
	...
	for (c = nexttiled(m->clients); c; c = nexttiled(c->next))
		...
		resize(c, m->wx + m->gappx, m->wy + m->gappx, m->ww - (2*m->gappx) - (2*c->bw), m->wh - (2*m->gappx) - (2*c->bw), 0);
}
